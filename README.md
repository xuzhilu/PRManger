# PRManager - 飞书智能PR审查助手


## 📖 项目简介

PRManager 是一个集成飞书机器人的智能代码审查系统，通过飞书与团队成员交互，自动分析 Git 分支合并提供专业的审查意见。所有AI推理在本地运行（基于Ollama），保护代码隐私。

## 🚀 快速开始

### 前置要求

| 工具 | 版本要求 | 说明 |
|------|---------|------|
| Python | 3.11.14 | 运行环境 |
| Git | 2.51.1 | 代码仓库管理 |

### 1️⃣ 安装项目

```bash
# 克隆项目
git clone https://github.com/StarlightGuide/PRManger.git
cd PRManger

# 安装Python依赖
pip install -r requirements.txt
```

### 2️⃣ 安装Ollama和模型

```bash
# 安装Ollama（参考 https://ollama.ai/download）

```

### 3️⃣ 安装ripgrep

```bash
# Windows
choco install ripgrep

# macOS
brew install ripgrep

# Ubuntu/Debian
sudo apt-get install ripgrep

# CentOS/RHEL
sudo yum install ripgrep
```

### 4️⃣ 配置系统

#### 编辑配置文件

编辑 `config/config.yaml`，填写实际配置：

#### 飞书配置说明(https://open.feishu.cn/)
- 创建企业自建应用
- `app_id,app_secret`:[应用凭证](images/凭证.png)
- [权限申请](images/权限申请.png)
- 运行 main.py 建立长连接
- [事件配置](images/事件配置.png)
- `verification_token,encrypt_key`:[订阅配置](images/订阅配置.png)
- 创建版本发布应用
- `admins`:打开API调试台(https://open.feishu.cn/api-explorer/)
           [管理员id](images/管理员id.png)

### 启动系统

```bash
python main.py
```


## 📱 使用说明

### 在飞书中使用

#### 1. 添加机器人

在飞书中搜索你的机器人名称并添加到对话

#### 2. 触发PR审查

发送以下格式的消息：

```
修改分支 merge 原分支
```

#### 3. 等待审查结果

系统会：
1. ✅ 确认收到请求
2. 📊 推送详细审查报告


### 核心组件

#### 📁 目录结构

```
PRManger/
├── main.py                 # 程序入口
├── config/
│   ├── config.yaml.example  # 配置模板
│   ├── config.yaml          # 系统配置
│   └── code_rules.yaml      # 编码规范
├── src/
│   ├── agents/            # 智能体模块
│   │   ├── listener_agent.py        # 消息监听
│   │   ├── decision_agent.py        # 策略决策
│   │   ├── code_analyzer_agent.py   # 代码分析
│   │   ├── context_collector_agent.py # 上下文收集
│   │   ├── feedback_agent.py        # 报告生成
│   │   └── git_review_agent.py      # Git操作
│   ├── analyzers/         # 分析器
│   │   └── project_analyzer/
│   │       ├── ast_parser.py           # Tree-sitter解析
│   │       ├── fast_file_searcher.py   # ripgrep搜索
│   │       └── code_parser.py          # 正则解析
│   ├── adapters/          # 外部适配器
│   │   ├── feishu_adapter.py  # 飞书SDK
│   │   └── git_adapter.py     # Git操作
│   ├── core/              # 核心逻辑
│   │   ├── workflow.py    # LangGraph工作流
│   │   └── state.py       # 状态管理
│   └── utils/             # 工具函数
│       ├── llm.py         # LLM调用
│       ├── config.py      # 配置加载
│       └── thread_safe_logger.py # 日志
└── logs/                  # 运行日志
```

#### 🔄 工作流程

1. **监听阶段** - 飞书机器人接收用户消息
2. **解析阶段** - 提取信息
3. **决策阶段** - 评估diff规模，选择审查策略
4. **分析阶段** - 迭代分析代码变更和影响范围
   - 第1轮：分析直接变更
   - 第2轮：收集相关上下文
   - 第3轮：深度影响评估
5. **报告阶段** - 生成结构化审查报告
6. **提交阶段** - 推送结果到飞书